...
라이브러리의 이용시에 문의가 코드 수정은 Issue와 PR 이외의 커뮤니케이션 수단은 사용할 수 없습니다.
...
10분이 기준이며, 둘 중 하나라도 이상이 생기면 바로 이전 배포 버전으로 롤백합니다.
...
`Forge`를 사용할 수 없는 상황일 경우, 서버 세팅은 패키지매니저만을 이용하며, 스크립트 히스토리를 Github의 README.md 파일 첫 문단에 기록합니다.
